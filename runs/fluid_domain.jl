...
# --------------- INPUTS AND OUTPUTS -------------------------------------------
# INPUT OPTIONS
simulation_name = "transition_comparison"               # Simulation to read
read_path       = "/home/flowlab/atagg/fall_2024/VPMOPT/runs/transition_comparison/" # Where to read simulation from

pfield_prefix   = "transition_comparison_pfield"      # Prefix of particle field files to read
staticpfield_prefix = "transition_comparison_staticpfield" # Prefix of static particle field files to read

nums            = [719]              # Time steps to process

# OUTPUT OPTIONS
save_path       = joinpath(read_path, "..", simulation_name*"-fdom_wing")  # Where to save fluid domain
output_prefix   = "transition_comparison_wing"             # Prefix of output files
...
# -------------- PARAMETERS ----------------------------------------------------
# Grid
#L               = 1.5                         # (m) reference length
#dx, dy, dz      = L/20, L/20, L/20          # (m) cell size in each direction
#X = [-14, 5, 0]
#Pmin            = L*[-0.50, -1.25, -1.25] + X   # (m) minimum bounds
#Pmax            = L*[ 2.00,  1.25,  1.25] + X  # (m) maximum bounds
#NDIVS           = ceil.(Int, (Pmax .- Pmin)./[dx, dy, dz])  # Number of cells in each direction
#nnodes          = prod(NDIVS .+ 1)          # Total number of nodes

#L               = 1.5                         # (m) reference length
#dx, dy, dz      = L/20, L/5, L/20          # (m) cell size in each direction
#X = [-10.5, 0, 1.0]
#Pmin            = L*[-0.50, -2, -1.25] + X   # (m) minimum bounds
#Pmax            = L*[ 2.00,  2,  1.25] + X  # (m) maximum bounds
#NDIVS           = ceil.(Int, (Pmax .- Pmin)./[dx, dy, dz])  # Number of cells in each direction
#nnodes          = prod(NDIVS .+ 1)          # Total number of nodes
...
dx, dy, dz      = L/20, L/5, L/20          # (m) cell size in each direction
X = [-12.5, 2.45, 0.0]
Pmin            = L*[-2.25, -1.25, -1.25] + X   # (m) minimum bounds
Pmax            = L*[5.00,  1.25,  1.25] + X  # (m) maximum bounds
NDIVS           = ceil.(Int, (Pmax .- Pmin)./[dx, dy, dz])  # Number of cells in each direction
nnodes          = prod(NDIVS .+ 1)          # Total number of nodes

Oaxis           = gt.rotation_matrix2(0, 0, 0.0)    # Orientation of grid
...
xr = range(Pmin[1], Pmax[1], NDIVS[1])
yr = range(Pmin[2], Pmax[2], NDIVS[2])
zr = range(Pmin[3], Pmax[3], NDIVS[3])
U = h5read("/home/flowlab/atagg/fall_2024/U_bemt_tail.h5", "U_bemt_tail")

Ux = x->FM.interp3d(FM.linear, xr, yr, zr, U[:,:,:,1], x[1], x[2], x[3])[1]
Uy = x->FM.interp3d(FM.linear, xr, yr, zr, U[:,:,:,2], x[1], x[2], x[3])[1]
Uz = x->FM.interp3d(FM.linear, xr, yr, zr, U[:,:,:,3], x[1], x[2], x[3])[1]
